...
PARTICLE_COUNT = 10;
V_MAX = 10;  # Maximum velocity change allowed.  Range: 0 >= V_MAX < CITY_COUNT
...
map__ = [];
CITY_COUNT = 38
TARGET = 10 # Number for algorithm to find.
XLocs = []
YLocs = []
...
def get_distance(firstCity, secondCity):
    cityA = map__[firstCity]
    cityB = map__[secondCity]
...
def initialize_map():
    file_name = "data.txt"
    data_file = open(f"{file_name}", "r")
    data = data_file.readlines()
    for line in data:
        x, y = map(float, line.strip().split())
        XLocs.append(math.ceil(x))
        YLocs.append(math.ceil(y))

    for i in range(CITY_COUNT):
        newCity = City()
        newCity.set_x(XLocs[i])
        newCity.set_y(YLocs[i])
        map__.append(newCity)
